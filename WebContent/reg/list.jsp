...
<html>
  <head>
	...
	<script type="text/javascript">
		...
		function deleteinfo()
		{
			$.post("regservice!delete.action",{'id':window.id},function(result){
				if(result.stateCode==1){
					layer.msg('删除成功！', {icon: 4});
					window.setTimeout(window.location.reload(), 4000); 
				}else{
					layer.msg('操作失败：'+result.message, {icon: 2});
				}
			},"json");
		}
		function reg(sid)
		{
			$.post("studentservice!reg.action",{'id':sid},function(result){
				if(result.stateCode==1){
					layer.msg(result.message,{icon: 4});
					window.location.reload();
				}else{
					layer.msg('注册失败：未知错误！', {icon: 2});
				}
			},"json");
		}
	</script>
            ...
            <div class="span3">
                ...
                <s:if test="#session.user.type==2">
	                ...
	                <ul id="legal-menus" class="nav nav-list collapse in">
	                  <li class="active"><a href="scanreg">查看申请</a></li>
	                </ul>
                </s:if>
            </div>
...
<div class="well">
    <table class="table">
      ...
      <tbody>
      	<s:iterator value="#session.regs" id="infos">
      		<tr>
	      		<td><%=++i%></td>
	      		<td><s:property value="#infos.no" /></td>
				<td><s:property value="#infos.name" /></td>
	          	<td><s:property value="#infos.date" /></td>
	          	<td>
	          		  <a href="#myModal22" role="button" data-toggle="modal2" onclick="reg(<s:property value="#infos.id" />)"><i class="icon-ok"></i></a>
...
<div class="modal small hide fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <div class="modal-body">
        <p class="error-text"><i class="icon-warning-sign modal-icon"></i>你确定要拒绝申请吗?</p>
    </div>
