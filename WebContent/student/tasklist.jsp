...
<html>
  <head>
	...
	<script type="text/javascript">
		...
		function deleteinfo()
		{
			$.post("taskservice!delete.action",{'id':window.id},function(result){
...
<div class="well">
    <table class="table">
      ...
      <tbody>
      	<s:iterator value="#session.tasks" id="infos">
      		<tr>
	      		<td><%=++i%></td>
	      		<td><a href="taskservice!detail.action?id=<s:property value="#infos.id" />"><s:property value="#infos.name" /></a></td>
				<td><a href="courseservice!detail.action?id=<s:property value="#infos.courseId" />"><s:property value="#infos.courseName" /></a></td>
    ...
    <div class="block-body">
                <div class="row-fluid" style="text-align: center;">
                    <div class="pull-left span4 unstyled">
                        <p>总记录数:<s:property value="taskPage.totalCount" /></p>
                    </div>
                    <div class="pull-left span4 unstyled">
                        <p>每页记录数：<s:property value="taskPage.everyPage" /></p>
                    </div>
                    <div>
                        <p>总页数：<s:property value="taskPage.totalPage" /></p>
                    </div>
                    </div>
    ...
    <ul>
		<s:if test="#request.taskPage.hasPrePage==true">
		<li><a href="scantask!frontPage.action?currentPage=<s:property value="taskPage.currentPage" />&style=1" class="btu">上一页</a></li>
		</s:if>
		<li><a href="#" style="color:#000;"><s:property value="taskPage.currentPage" /></a></li>
		<s:if test="#request.taskPage.currentPage+1<=#request.taskPage.totalPage">
		<li><a href="scantask!nextPage.action?currentPage=<s:property value="taskPage.currentPage" />&style=1" class="btu"><s:property value="taskPage.currentPage+1" /></a></li>
		</s:if>
		<s:if test="#request.taskPage.currentPage+2<=#request.taskPage.totalPage">
		<li><a href="scantask!nextPage.action?currentPage=<s:property value="taskPage.currentPage+1" />&style=1" class="btu"><s:property value="taskPage.currentPage+2" /></a></li>
		</s:if>
		<s:if test="#request.taskPage.currentPage+3<=#request.taskPage.totalPage">
		<li><a href="scantask!nextPage.action?currentPage=<s:property value="taskPage.currentPage+2" />&style=1" class="btu"><s:property value="taskPage.currentPage+3" /></a></li>
		</s:if>
		<s:if test="#request.taskPage.hasNextPage==true">
		<li><a href="scantask!nextPage.action?currentPage=<s:property value="taskPage.currentPage" />&style=1" class="btu">下一页</a></li>
		</s:if>
	</ul>
</div>
