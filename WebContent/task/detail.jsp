...
<div class="well" style="min-height:500px;">
	...
	<hr>
	<p>课程号：${task.courseNo}</p>
	<p>课程名称：${task.courseName}</p>
	<p>指导教师：${task.teacherName}</p>
	<p>开始时间：${task.startTime}</p>
	<p>结束时间：${task.endTime}</p>
	<hr>
	<h4 style="text-align:center;">作业内容</h4>
    <p>
    <% out.println(((Task)request.getAttribute("task")).getContent());%>
    </p>

    <p>附件下载:</br>
    	<%
    	String str=((Task)request.getAttribute("task")).getAttach();
    ...
    <s:if test="#session.user.type==1">
		<s:if test="#request.taskMap==null">
			<div style="text-align:center;">
				<h4 style="text-align:center;">作业状态：未提交</h4>
				<a class="btn btn-primary" href="taskmapservice!edit.action?id=${task.id}">提交作业</a>
			</div>		
		</s:if>
		<s:elseif test="#request.taskMap.state!=1">
			<div style="text-align:center;">
				<h4 style="text-align:center;">作业状态：已提交</h4>
				<a class="btn btn-primary" href="taskmapservice!edit.action?id=${task.id}">修改作业</a>
			</div>
		</s:elseif>
		<s:else>
			<h4 style="text-align:center;">成绩</h4>
			<h2 style="color:red;text-align:center;">${taskMap.score}</h2>
		</s:else>
    </s:if>
    <s:if test="#session.user.type==2">
    	<div style="text-align:center;">
				<a style="background-color:#0f0;" class="btn" href="scancoursemap?courseId=${task.courseId}&id=${task.id}">查看作业完成情况</a>
			</div>
    </s:if>
</div>
