  ...
  <head>
	...
	function submitData(){
		...
		if(title==null || title==''){
			...
			layer.msg('请上传作业！', {icon: 2});
		}else{
			$.post("taskmapservice!add.action",{'taskMap.content':title,'taskMap.attach':attach,'taskMap.taskId':taskId},function(result){
				if(result.stateCode==1){
					layer.msg(result.message, {icon: 4});
					window.setTimeout('scantask', 5000); 
...
<div class="well">
    ...
    <form id="tab">
        ...
        <input type="hidden" id="taskId" value='<s:property value="#request.task.id"/>'>
    </form>
      </div>
